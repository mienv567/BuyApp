...
-(void)dismiss{
    ...
    if ([self selectedObject]!=nil  && selectedRow > -1) {
        selectedRow = [_pickerViewArray indexOfObject:[self selectedObject]];
    }else{
        selectedRow = [[_pickerViewArray objectAtIndex:0] integerValue];
    }
    
    if (self.objectToStringConverter == nil) {
        self.onDismissCompletion ([_pickerViewArray objectAtIndex:selectedRow]);
    } else{
...
-(void)initializePickerViewInView: (UIView *)view
  ...
  NSString * chosenObject = options[MMselectedObject];
...
//  _pickerViewBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(dismiss)];
  ...
  */
  ...
  //Add pickerView
  _pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0.0, 44.0, K_WIDTH, 216.0)];
