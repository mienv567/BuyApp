...
- (BOOL)isVaild{
    if(self.expire){
        return [self.expire compare:[NSDate date]] >= 0;
    }
    return YES;
}
...
NSInteger const kImageCacheMaxNum = 1000;
...
- (id)init{
    if (self = [super init]){
        ...
        NSString *dbPath = [dbDir stringByAppendingPathComponent:@"com.hackemist.SDWebImageCache.default/cache.db"];
        //创建数据库实例 db  这里说明下:如果路径中不存在"cache.db"的文件,sqlite会自动创建"cache.db"
        _db= [FMDatabase databaseWithPath:dbPath] ;
        if (_db.open) {
            //创建表
            [_db executeUpdate:@"create table if not exists cachetb (cid integer primary key autoincrement,cdir text,ckey text,cvalue text,cexpire text)"];
            //创建索引
...
- (void)storeValue:(NSString *)value dir:(NSString *)dir key:(NSString *)key{
    [self storeValue:value dir:dir key:key expire:[NSDate dateWithTimeIntervalSinceNow:24 * 3600]];
}
...
- (void)storeValue:(NSString *)value dir:(NSString *)dir key:(NSString *)key expire:(NSDate *)expire{
    ...
    //存储数据
    [_db executeUpdate:@"insert into cachetb (cdir, ckey, cvalue, cexpire) VALUES (?,?,?,?)", newDir, newKey, value, [expire toStrFormat:@"yyyy-MM-dd HH:mm:ss"]];
    //从二级缓存中移除
...
- (ASCacheObject *)readDicFiledsWithDir:(NSString *)dir key:(NSString *)key{
    ...
    if (!objCached) {
        ...
        if ([rs next]) {
            ...
            //过期时间戳
            c.expire = [NSDate dateFromString:[rs stringForColumn:@"cexpire"] format:@"yyyy-MM-dd HH:mm:ss"];
            //保存到二级缓存中
