//
//  UITableViewCell+GSMasonryAutoCellHeight.h
//  CellAutoHeightDemo
//
 ...
 */
typedef void(^GSCellBlock)(UITableViewCell *sourceCell);

typedef NSDictionary *(^GSCacheHeight)();
 ...
 *	唯一键，通常是数据模型的id，保证唯一
 */
FOUNDATION_EXTERN NSString *const kGSCacheUniqueKey;
 ...
 */
FOUNDATION_EXTERN NSString *const kGSCacheStateKey;
 ...
 */
FOUNDATION_EXTERN NSString *const kGSRecalculateForStateKey;
 ...
 *	指定为缓存tableview缓存 
 */
FOUNDATION_EXTERN NSString *const kGSCacheForTableViewKey;
 ...
 */
@interface UITableViewCell (GSMasonryAutoCellHeight)
 ...
 *
 * 且必须指定GS_lastViewInCell才能生效
 ************************************************************************/

/**
 * 必传设置的属性，也就是在cell中的contentView内最后一个视图，用于计算行高
 * 例如，创建了一个按钮button作为在cell中放到最后一个位置，则设置为：self.GS_lastVieInCell = button;
 * 即可。
 * 默认为nil，如果在计算时，值为nil，会crash
 */
@property (nonatomic, strong) UIView *GS_lastViewInCell;

/**
 * 可选设置的属性，默认为0，表示指定的GS_lastViewInCell到cell的bottom的距离
 * 默认为0.0
 */
@property (nonatomic, assign) CGFloat GS_bottomOffsetToCell;
